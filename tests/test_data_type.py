...
class TestDataFactory(unittest.TestCase):
    ...
    def test_create_data_type(self):
        ...
        attr = self.factory.create_data_type('attribute')
        self.assertEqual(attr.type._type, 'attribute')
...
class TestPSData(unittest.TestCase):
    def setUp(self):
        ...
        self.attribute = PSData(Attribute)
        ...
        self.fp_attribute = os.path.join(THIS_DIR, 'test_data/attribute/')
    ...
    def test_get_data(self):
        ...
        self.assertEqual(len(self.ticket.raw), 800)

        self.ticket.get_data(fys=[20, 19], path=self.fp_ticket)
        self.assertEqual(len(self.ticket.raw), 1800)

        self.donor.get_data(fys=13, path=self.fp_donor)
        self.assertEqual(len(self.donor.raw), 1000)
        ...
        self.attribute.get_data(fys=None, path=self.fp_attribute)
        self.assertEqual(len(self.attribute.raw), 4)
